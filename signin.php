...
$check_user = mysqli_query($connect, "SELECT * FROM `users` WHERE `login`='$login' and `password`='$password'");//проверка логина и пароля
if(mysqli_num_rows($check_user)>0){
    
    $user = mysqli_fetch_assoc($check_user); //внос данных в сессию юзер
    $_SESSION['user'] = [
    "id" => $user['id_user'],
    "fio" => $user['fio'],
    "email" => $user['email'],
